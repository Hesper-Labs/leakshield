...
- Versioning + adversarial test gate: a new category that would have allowed a previously
  blocked sample is rejected at save time. (See `setup-wizard.md` for the broader test harness
  flow.)
...
like master provider keys, and never logged or sent in OTel traces.
