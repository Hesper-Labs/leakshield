...
This document is a living overview of how the gateway, inspector, and admin panel fit together.
For the original design discussion and tradeoffs see the approved plan
(`bir-proje-yapmak-istiyorum-crystalline-token.md`).

## Components

| Component | Tech | Responsibility |
|---|---|---|
| `gateway` | Go (chi, pgx, Redis, gRPC) | Public proxy + admin REST API + worker. Multi-protocol native endpoints (`/openai/*`, `/anthropic/*`, `/google/*`, `/azure/*`). |
| `inspector` | Python (asyncio, gRPC, Presidio, optional vLLM/Ollama) | DLP decisions per request. Pluggable strategies and backends. |
| `panel` | Next.js 15 + TypeScript + Tailwind v4 + shadcn/ui + Recharts + Monaco | Admin web app — onboarding wizard, providers, users, keys, policy editor, analytics, live audit log. |
...
| Universal router | `/v1/chat/completions` (OpenAI format, optional) | LiteLLM-style; routes by virtual key policy or `model` heuristic |
...
The strategy is chosen per tenant from the admin panel; the model is the admin's choice too.
...
- **`hybrid`** — Presidio (regex / NER, including Turkish recognizers) for fast classification,
  escalating ambiguous cases to a user-chosen LLM.
...
LeakShield does **not** download models for you. The inspector's `mock` backend lets the gateway
...
- **Secrets**: Vault / KMS required in `--prod` mode; the gateway refuses to start with an
